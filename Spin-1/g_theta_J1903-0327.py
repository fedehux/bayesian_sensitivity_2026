...
# === J1903+0327 PARAMETERS ===
# Values sourced from Freire et al. (2010) and NANOGrav
pb_sec  = 95.2 * 24 * 3600         # Orbital period in seconds
wb      = 2 * np.pi / pb_sec      # Orbital frequency (2*pi/Pb)
a1      = 105.6      # Projected semi-major axis in light-seconds
w_rad   = 141.7 * (np.pi / 180)      # Argument of periastron in radians
ecc     = 0.437       # Eccentricity
iota    = np.arcsin(0.976)      # Inclination angle in radians
m1      = 1.67      # Pulsar mass in Solar Masses
m2      = 1.03      # Companion mass in Solar Masses

# --- Timing & Observation Specs ---
toa_precision = 6.24e-6  # TOA precision (epsilon) in seconds
...
np.savez("g_results_eccentric_J1903_0327.npz", curves=g_results_stack, masses=mass_grid)
